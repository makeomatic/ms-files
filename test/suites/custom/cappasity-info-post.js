  ...
  describe('get embedded info', function modelSuite() {
    it('should be able to set embed code', function test() {
      ...
      const expectedAttrs = flatstr(`
          ...
          + '&analytics={{ analytics }}&uipadx={{ uipadx }}&uipady={{ uipady }}&autorotate={{ autorotate }}'
          + '&autorotatetime={{ autorotatetime }}&autorotatedelay={{ autorotatedelay }}&autorotatedir={{ autorotatedir }}'
          + '&hidefullscreen={{ hidefullscreen }}&hideautorotateopt={{ hideautorotateopt }}&hidesettingsbtn={{ hidesettingsbtn }}"')
        .replace(/\s+/g, ' ')
        .trim();
